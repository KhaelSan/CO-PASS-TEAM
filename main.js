...
const starts = async (copass = new WAConnection()) => {
    ...
    fs.existsSync('./session.json') && copass.loadAuthInfo('./session.json')
    ...
    await copass.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./session.json', JSON.stringify(copass.base64EncodedAuthInfo(), null, '\t'))
    
copass.on('group-participants-update', async (anu) => {
	try {
		const mdata = await copass.groupMetadata(anu.jid)
                const behys = mdata.participants.length
		console.log(anu)
		if (anu.action == 'add') {
                if (gcrevoke.includes(anu.jid)) {
                copass.revokeInvite(anu.jid)
                }
...
pushnem = copass.contacts[num] != undefined ? copass.contacts[num].notify = undefined ? PhoneNumber('+' + num.replace('@s.whatsapp.net', '')).getNumber('international') : copass.contacts[num].notify || copass.contacts[num].vname : PhoneNumber('+' + num.replace('@s.whatsapp.net', '')).getNumber('international')
			try {
				ppimg = await copass.getProfilePicture(`${anu.participants[0].split('@')[0]}@c.us`)
			} catch {
				...
				.on('error', () => copass.sendMessage(m.chat, 'error', MessageType.text))
				.on('exit', () => {
			copass.sendMessage(mdata.id, fs.readFileSync('hasil.jpg'), MessageType.image, {caption: `*Hay!!,Jangan Lupa Baca Deskripsi Dan Patuhi Peraturan Grup* @${num.split('@')[0]}`, contextInfo: { mentionedJid: [num] }})
			})
...
pushnem = copass.contacts[num] != undefined ? copass.contacts[num].notify = undefined ? PhoneNumber('+' + num.replace('@s.whatsapp.net', '')).getNumber('international') : copass.contacts[num].notify || copass.contacts[num].vname : PhoneNumber('+' + num.replace('@s.whatsapp.net', '')).getNumber('international')
			try {
				...
				.on('error', () => copass.sendMessage(m.chat, 'error', MessageType.text))
				.on('exit', () => {
			copass.sendMessage(mdata.id, fs.readFileSync('hasil.jpg'), MessageType.image, {caption: `*Sayonara👋🏻*@${num.split('@')[0]}\n`, contextInfo: { mentionedJid: [num] }})
			})
		}
	} catch (e) {
		console.log(e)
...
copass.on('chat-update', async (message) => {
        require('./index.js')(copass, message)
})
antidel = true
copass.on('message-delete', async (m) => {
...
copass.sendMessage(m.key.remoteJid, `\`\`\`「 Anti Delete 」\`\`\`
•> Nama : @${m.participant.split("@")[0]}
•> Waktu : ${jam} ${week} ${calender}
•> Type : ${type}`, MessageType.text, {quoted: m.message, contextInfo: {"mentionedJid": [m.participant]}})

copass.copyNForward(m.key.remoteJid, m.message)
})
antical = true
copass.on("CB:Call", json => {
if (antical === false) return
let call;
calling = JSON.parse(JSON.stringify(json))
call = calling[1].from
copass.sendMessage(call, `*Sorry ${copass.user.name} can't receive calls.*\n*Call = Block!*`, MessageType.text)
.then(() => copass.blockUser(call, "add"))
})
